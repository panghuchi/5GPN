...
[[ "${chinalist_block}" == *'awk -F/'* ]] || { echo "ChinaList parser must use bulk awk extraction." >&2; exit 1; }
[[ "${chinalist_block}" != *'trim_domain'* ]] || { echo "ChinaList parser must not call trim_domain once per line." >&2; exit 1; }
