...
assert_contains "${install}" 'activate_direct_egress()' 'direct egress activation function'
...
assert_contains "${install}" 'set_egress_command()' 'set-egress command'
assert_contains "${install}" 'delete_exit_command()' 'delete-exit command'
assert_contains "${install}" '--add-exit|--add-exits)' 'add-exits alias dispatch'
assert_contains "${install}" '--list-exits)' 'list-exits CLI dispatch'
assert_contains "${install}" '--set-exit)' 'set-exit CLI dispatch'
assert_contains "${install}" '--set-egress)' 'set-egress CLI dispatch'
assert_contains "${install}" '--set-direct)' 'set-direct CLI dispatch'
assert_contains "${install}" '--delete-exit|--del-exit|--remove-exit)' 'delete-exit CLI dispatch'
...
assert_contains "${readme}" './install.sh --set-egress direct' 'README documents set-egress direct'
assert_contains "${readme}" './install.sh --delete-exit' 'README documents delete-exit'
