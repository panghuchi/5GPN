...
[[ "${install}" == *'for svc in mosdns sniproxy 5gpn-tcp-proxy quic-proxy china-dns-race-proxy'* ]] || { echo "install.sh --status must include china-dns-race-proxy and 5gpn-tcp-proxy." >&2; exit 1; }
