...
[[ "${install}" == *'for svc in dnsdist mosdns sniproxy 5gpn-tcp-proxy quic-proxy china-dns-race-proxy'* ]] || { echo "install.sh --status must include dnsdist, china-dns-race-proxy, and 5gpn-tcp-proxy." >&2; exit 1; }
